  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-neutral-100">Decision Rules</h1>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-base font-semibold text-neutral-100">{rs.name}</h3>
                      {rs.is_active ? (
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {Object.entries(rs.weights ?? {})
                    ...
                    .map(([k, v]) => (
                      <span
                        key={k}
                        className="rounded-md border border-neutral-800 bg-neutral-950/50 px-2 py-0.5 text-xs text-neutral-400"
                      >
                        {k.replace(/_/g, ' ')}: <span className="text-neutral-200">{v}</span>
                      </span>
                    ))}
                </div>
              ...
              <div className="space-y-3">
                {Object.keys(weights).map((k) => (
                  <div key={k} className="flex items-center gap-3">
                    <label className="w-44 shrink-0 text-sm capitalize text-neutral-300">
                      {k.replace(/_/g, ' ')}
                    </label>
                    <input
                      type="range"
      ...
      >
        <p className="text-sm text-neutral-300">
          Delete rule set{' '}
          <span className="font-semibold text-neutral-100">{confirmDelete?.name}</span>? This cannot
          be undone.
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-4">
        <div>
          <div className="text-xs text-neutral-500">Alerts evaluated</div>
          <div className="text-xl font-semibold text-neutral-100">{total}</div>
        </div>
        {sim.projected_deflection_rate != null && (
          <div>
            <div className="text-xs text-neutral-500">Projected deflection rate</div>
      ...
      {entries.length > 0 && (
        <div className="space-y-2">
          {entries.map(([k, n]) => (
            <div key={k} className="flex items-center gap-3">
              <span className="w-32 shrink-0 text-sm capitalize text-neutral-400">
                {k.replace(/_/g, ' ')}
              </span>
              <div className="h-5 flex-1 overflow-hidden rounded bg-neutral-900">
                <div
                  className={`h-full ${
                    dispoTones[k] === 'green'
                      ? 'bg-emerald-500/70'
              ...
              <span className="w-10 text-right text-sm font-medium text-neutral-200">{n}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {sim.dispositions && sim.dispositions.length > 0 && (
        <div className="max-h-48 space-y-1 overflow-y-auto pr-1">
          {sim.dispositions.slice(0, 40).map((d) => (
            <div
              key={d.alert_id}
              className="flex items-center justify-between rounded-md border border-neutral-800 bg-neutral-950/50 px-2.5 py-1.5 text-xs"
            >
              <span className="font-mono text-neutral-500">{d.alert_id.slice(0, 8)}</span>
              <span className="text-neutral-400">{d.reason_code || '—'}</span>
              <Badge tone={dispoTones[d.recommendation] ?? 'slate'}>{d.recommendation}</Badge>
              <span className="text-neutral-300">{d.score}</span>
            </div>
          ))}
        </div>
      )}
...
function FormField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="mb-1 block text-xs font-medium uppercase tracking-wide capitalize text-neutral-500">
        {label}
      </span>
      {children}
    </label>
