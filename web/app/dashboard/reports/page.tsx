    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className={`rounded-lg px-3 py-1.5 text-sm font-medium transition-colors ${kindFilter === '' ? 'bg-orange-600 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}
            >
              All
            </button>
            {KINDS.map((k) => (
              <button
                ...
                className={`rounded-lg px-3 py-1.5 text-sm font-medium transition-colors ${kindFilter === k.value ? 'bg-orange-600 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}
              >
                {k.label}
              </button>
            ))}
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-200 placeholder-slate-500 focus:border-orange-500 focus:outline-none sm:w-64"
          />
        </CardHeader>
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            <label className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-slate-500">Report kind</label>
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              {KINDS.map((k) => (
                <button
                  ...
                  className={`rounded-lg border px-3 py-3 text-left transition-colors ${genKind === k.value ? 'border-orange-500 bg-orange-500/10' : 'border-slate-700 bg-slate-950 hover:border-slate-600'}`}
                >
                  <div className="text-sm font-semibold text-slate-100">{k.label}</div>
                  <div className="mt-1 text-xs text-slate-400">{k.desc}</div>
                </button>
              ))}
            </div>
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-slate-500">Title (optional)</label>
            <input
              value={genTitle}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-slate-500">Period start</label>
              <input
                type="date"
                value={genStart}
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-slate-500">Period end</label>
              <input
                type="date"
                value={genEnd}
        ...
        {detailLoading ? (
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="text-sm text-slate-500">Generated {fmtDateTime(selected.created_at)}</span>
            </div>
...
function ReportDataView({ data }: { data: any }) {
  if (data == null) {
    return <p className="text-sm text-slate-400">No report data captured.</p>
  }
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {scalarMetrics.map(([k, v]) => (
            <div key={k} className="rounded-lg border border-slate-800 bg-slate-950 p-3">
              ...
              <div className="mt-1 text-lg font-semibold text-slate-100">{String(v)}</div>
            </div>
          ))}
        </div>
      )}
      {nested.map(([k, v]) => (
        <div key={k}>
          <div className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-slate-500">{k.replace(/_/g, ' ')}</div>
